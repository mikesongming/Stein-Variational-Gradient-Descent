...
class BayesianLR:
    ...
    def dlnprob(self, theta):
        ...
        y_hat = np.where(
            coff >= 0, # condition
            1 / (1 + np.exp(-coff)), # For positive values
            np.exp(coff) / (1 + np.exp(coff)) # For negative values
        )
        ...
        dalpha = d / 2.0 - wt + (self.a0 - 1) - alpha / self.b0 + 1  # the last term is the jacobian term
    ...
    def evaluation(self, theta, X_test, y_test):
        ...
        for t in range(M):
            coff = np.multiply(y_test, np.sum(-1 * np.multiply(np.tile(theta[t, :], [n_test, 1]), X_test), axis=1))
...
if __name__ == '__main__':
    ...
    theta = SVGD().update(x0=theta0, lnprob=model.dlnprob, bandwidth=-1, n_iter=6000, stepsize=0.05, alpha=0.9, debug=False)

    print('svgd: ', theta[:, :-1].mean(axis=0), np.exp(theta[:, -1]).mean())
